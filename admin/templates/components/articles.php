  ...
  <?php if (getQueryParam('section') === 'nuevo') : ?>
    <form class="form" action="" method="POST">
      ...
      <div class="form-group">
        ...
        <input name="article_code" id="article_code" type="text" class="<?php fieldHasError('article_code', 'error') ?>" class="<?php fieldHasError('article_code', 'error') ?>" value="<?php getPreformData('article_code', '') ?>">
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
        ...
        <input name="article_price" id="article_price" type="number" class="<?php fieldHasError('article_price', 'error') ?>" value="<?php getPreformData('article_price', '') ?>">
      </div>
      <div class="form-group">
        <label for="article_img_url">Imagenes (URL):</label>
        <input name="article_img_url" id="article_img_url" type="text" class="<?php fieldHasError('article_img_url', 'error') ?>" value="<?php getPreformData('article_img_url', '') ?>">
      </div>
      <div class="form-group">
        <label for="article_category">Categoría Padre:</label>
        <select name="article_category" id="article_category" type="text" value="<?php getPreformData('article_category', '') ?>" <?php empty($categories) ? bind('disabled') : '' ?>>
          <?php if (!empty($categories)) : ?>
            <option value="no-category"> - </option>
            <?php foreach ($categories as $category) : ?>
              <option value="<?php bind($category->title) ?>+<?php bind($category->id) ?>" <?php bind((isset($cat_parent) && $category->id == $cat_parent->id) ? 'selected' : '') ?>>
      ...
      <div class="form-group collapsable-box closed" data-height="0">
        ...
        <input name="article_price_offer" id="article_price_offer" type="number" class="<?php fieldHasError('article_price_offer', 'error') ?>" value="<?php getPreformData('article_price_offer', '') ?>">
      </div>
            ...
            <?php for ($i = 0; $i < @count($articles); $i++) : ?>
              <div class="table-row <?php bind(($i % 2 === 0) ? 'background' : '') ?>">
                ...
                <div class="actions list-admin-buttons">
                  <a class="restore-button restore-article-button" data-action="<?php bind(ACTION_RESTORE_ARTICLE) ?>" data-type="restore-article" data-id="<?php bind($articles[$i]->id) ?>"><i class="fas fa-trash-restore"></i>
                    Restaurar</a>
                </div>
              </div>
            <?php endfor ?>
