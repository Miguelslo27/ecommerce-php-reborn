<form action="" class="shipping-info" method="POST">
  <fieldset>
    ...
    <div class="shipping-info-form collapsable <?php bind((shippingInfoFormHasErrors() && empty(getPreformData('copy-billing-address', '')) || shippingInfoIsIncomplete()) ? 'open' : 'closed') ?>">
      ...
      <div class="form-line <?php fieldHasError('shipping_address', 'error') ?>">
        ...
        <input type="text" name="shipping_address" id="shipping_address" value="<?php bind(oneOf(getGlobal('shipping_address'), getCurrentUser()->address)) ?>">
      </div>

      <div class="form-group">
        <label for="shipping_state" class="<?php fieldHasError('shipping_state', 'error') ?>">Departamento *</label>
        <input type="text" class="<?php fieldHasError('shipping_state', 'error') ?>" name="shipping_state" id="shipping_state" value="<?php bind(oneOf(getGlobal('shipping_state'), getCurrentUser()->state)) ?>">
        <label for="shipping_city" class="align-center <?php fieldHasError('shipping_city', 'error') ?>">Localidad *</label>
        <input type="text" class="<?php fieldHasError('shipping_city', 'error') ?>" name="shipping_city" id="shipping_city" value="<?php bind(oneOf(getGlobal('shipping_city'), getCurrentUser()->city)) ?>">
      </div>

      <div class="form-group">
        <label for="shipping_zipcode" class="align-center <?php fieldHasError('shipping_zipcode', 'error') ?>">Código postal</label>
        <input type="text" class="<?php fieldHasError('shipping_zipcode', 'error') ?>" name="shipping_zipcode" id="shipping_zipcode" value="<?php bind(oneOf(getGlobal('shipping_zipcode'), '')) ?>">
      </div>
    </div>
